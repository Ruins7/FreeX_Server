...
public class H2currenyPool {
	...
	private static final String DB_PASSWORD = "pass";// this need to be stored
														// somewhere
	...
	public static void update_notrade(Traderinfo sellinfo) throws SQLException {
		...
		ResultSet rs = stmt
				.executeQuery("select * from Currency_loc WHERE trade_id="
						+ (cur_id) + "");
		...
		for (int i = 0; i < 30; i++) {
			ResultSet rs_p = stmt
					.executeQuery("select * from Currency_pool WHERE " + "id="
							+ (pool_loc + i) + "");
			rs_p.next();
			int rs_p_amount = rs_p.getInt("amount");
			if (rs_p.getInt("user_id") == user_id
					&& (rate == rs_p.getInt("ex_rate"))) {
				stmt.execute("UPDATE Currency_pool SET (amount) =("
						+ (rs_p_amount + amount_t) + ") " + "WHERE id ="
						+ (pool_loc + i) + "");
				stmt.close();
				connection.commit();
				connection.close();
	...
	private static void createpool(int[] trade_id) throws SQLException {
		 ...
		 * " user_id " + rs.getInt("user_id") +" ex_rate " +
		 * rs.getInt("ex_rate")+ time " + rs.getInt("time")); }
		 */
		stmt.close();
		connection.commit();
		connection.close();
	...
	// The pool table length will be added dynamically with respect to the
	// number of currency
	private static void createpoolloc(int[] trade_id) throws SQLException {
		...
		for (int i = 0; i < trade_id.length; i++) {
			...
			stmt.execute("INSERT INTO Currency_loc( trade_id,pool_loc,pool_preloc)VALUES( "
					+ trade_id[i] + ",0,0)");

		}
		/*
		 * ResultSet rs = stmt.executeQuery("select * from Currency_loc");
		 * System.out.println("H2 In-Memory Database Currency_loc Table"); while
		 * (rs.next()) { System.out.println("Id " + rs.getInt("id") +
		 * " trade_id " + rs.getInt("trade_id") +" pool_loc " +
		 * rs.getInt("pool_loc")+" pool_preloc " + rs.getInt("pool_preloc")); }
		 */
		stmt.close();
		connection.commit();
		connection.close();
	...
	private static Connection getPoolConnection() throws SQLException {
		...
		JdbcConnectionPool dbConnection = JdbcConnectionPool.create(
	...
	// test insert input
	private static void updatetable(int[][] cur_loc, int[][] cur_pool)
			throws SQLException {
		Connection connection = getPoolConnection();
		Statement stmt = null;
		...
		try {
			...
			for (int i = 0; i < cur_loc.length; i++) {
				...
				stmt.execute("UPDATE Currency_loc SET( trade_id,pool_loc,pool_preloc) =("
						...
						+ cur_loc[i][1]
						+ ", "
						+ cur_loc[i][2] + ")WHERE id=" + (i + index + 1) + "");
			}
	...
	public static void readtable() throws SQLException {
		...
		while (rs.next()) {
			System.out.println("Id " + rs.getInt("id") + " trade_id "
					+ rs.getInt("trade_id") + " pool_loc "
					+ rs.getInt("pool_loc") + " pool_preloc "
					+ rs.getInt("pool_preloc"));
		}
		ResultSet rs1 = stmt.executeQuery("Select * from Currency_pool");
		System.out.println("H2 In-Memory Database Currency_pool Table");
		while (rs1.next()) {
			System.out.println("Id " + rs1.getInt("id") + " user_id "
					+ rs1.getInt("user_id") + " ex_rate "
					+ rs1.getInt("ex_rate") + "amount" + rs1.getInt("amount")
	...
	// table mode is either one or two, 1= upper half of the currency, 2=lower
	// half
	// depend on timing, either one can be current and the other one will be
	// previous
	public static Traderinfo poolQuary(int cur_id, double amount_t1)
			throws SQLException {
		...
		System.out.println("amount_sell: " + amount_sell + " amount_t: "
				+ amount_t);
		double amount_f = 0;
		...
		// amount_sell =amount buy, check next row,update currency location
			...
			if (amount_next == 0) {
				...
				stmt.execute("UPDATE Currency_loc SET (pool_loc) =("
						+ pool_locn + ") " + "WHERE trade_id =" + cur_id + "");
			}
		}
		stmt.close();
		connection.commit();
		...
		Traderinfo seller = new Traderinfo(user_id, cur_id, rate_f1, amount_f,
				user_time);
	...
	private static int[][] readtxtfile(String filepath, int filenum) {
		...
		if (filenum == 1) {
			...
			System.out.println(a[21][0] + " " + a[1][1] + " " + a[1][2] + " "
					+ a[1][3]);
			return a;
		} else {
			int[][] b = new int[6][3];
	 ...
	 * Connection connection = getDBConnection(); PreparedStatement
	 * createPreparedStatement = null; PreparedStatement insertPreparedStatement
	 * = null; PreparedStatement selectPreparedStatement = null;
	 * 
	 * // String CreateQuery =
	 * "CREATE TABLE Currency_loc(id int primary key, trade_id int, pool_loc int,"
	 * // + "pool_preloc int)" ; String InsertQuery = "INSERT INTO Currency_loc"
	 * + "(id, trade_id,pool_loc,pool_preloc) values" + "(?,?,?,?)"; String
	 * SelectQuery = "select * from Currency_loc";
	 * 
	 ...
	 * insertPreparedStatement.setInt(1, 1); insertPreparedStatement.setInt(2,
	 * 1); insertPreparedStatement.setInt(3, 1);
	 ...
	 * System.out.println(
	 * "H2 In-Memory Database inserted through PreparedStatement"); while
	 * (rs.next()) { System.out.println("Id " + rs.getInt("id") + " trade_id " +
	 * rs.getInt("trade_id") +" pool_loc " +
	 * rs.getInt("pool_loc")+" pool_preloc " + rs.getInt("pool_preloc")); }
	 ...
	 * connection.close(); } // TODO Auto-generated method stub
	 * 
	 * }
	 */
